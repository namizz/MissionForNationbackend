    ...
    middleware(req, res, (err) => {
      ...
      if (err instanceof multer.MulterError) {
        if (err.code === "LIMIT_FILE_SIZE") {
          return res
            .status(400)
            .json({ error: "File too large. Max size is 200MB" });
        }
...
function getUploadedMediaFiles(req) {
  ...
  for (const file of files) {
    if (!imageFile && file.mimetype && file.mimetype.startsWith("image/"))
      imageFile = file;
    if (!videoFile && file.mimetype && file.mimetype.startsWith("video/"))
      videoFile = file;
  }
...
async function userCanPostToRegion(userId, regionId) {
  const membership = await db.query(
    "SELECT 1 FROM user_regions WHERE user_id=$1 AND region_id=$2",
    [userId, regionId],
  );
...
async function validateChurchIdsForRegion(churchIds, regionId) {
  ...
  const c = await db.query(
    "SELECT id FROM churches WHERE region_id=$1 AND id = ANY($2::uuid[])",
    [regionId, uniqueChurchIds],
  );
  return c.rowCount === uniqueChurchIds.length;
}

async function getChurchById(churchId) {
  const result = await db.query("SELECT * FROM churches WHERE id=$1", [
    churchId,
  ]);
...
router.get("/regions/:id", async (req, res) => {
  const { id } = req.params;
  const r = await db.query("SELECT * FROM regions WHERE id=$1", [id]);
  if (r.rowCount !== 1)
    return res.status(404).json({ error: "Region not found" });
  return res.json({ region: r.rows[0] });
});
...
router.put(
  ...
  async (req, res) => {
    ...
    if (result.rowCount === 0)
      return res.status(404).json({ error: "Region not found" });
    return res.json({ ok: true, region: result.rows[0] });
  },
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!name || !resolvedRegionId)
        return res.status(400).json({ error: "Missing name or region_id" });

      const rr = await db.query("SELECT id FROM regions WHERE id=$1", [
        resolvedRegionId,
      ]);
      if (rr.rowCount !== 1)
      ...
      if (req.user.role === "regional_admin") {
        const allowedRegion = await userCanPostToRegion(
          req.user.id,
          resolvedRegionId,
        );
        if (!allowedRegion)
          return res.status(403).json({ error: "Forbidden for this region" });
      }

      const normalizedServiceTimes = normalizeJsonArray(
        serviceTimes,
        "serviceTimes",
      );
      const normalizedAnnouncements = normalizeJsonArray(
        announcements,
        "announcements",
      );
      ...
      if (err.code === "23505") {
        return res
          .status(409)
          .json({ error: "Church id already exists for this region" });
      }
...
router.get("/churches", async (req, res) => {
  ...
  if (region_id) {
    const c = await db.query(
      "SELECT * FROM churches WHERE region_id=$1 ORDER BY created_at DESC",
      [region_id],
    );
    ...
    try {
      ...
      const normalizedServiceTimes =
        serviceTimes === undefined
          ? existing.service_times
          : normalizeJsonArray(serviceTimes, "serviceTimes");
      const normalizedAnnouncements =
        announcements === undefined
          ? existing.announcements
          : normalizeJsonArray(announcements, "announcements");
      const normalizedEvents =
        events === undefined
          ? existing.events
          : normalizeJsonArray(events, "events");
      const normalizedMinistries =
        ministries === undefined
          ? existing.ministries
          : normalizeJsonArray(ministries, "ministries");
      const normalizedGallery =
        gallery === undefined
          ? existing.gallery
          : normalizeJsonArray(gallery, "gallery");
      const normalizedPastor =
        pastor === undefined
          ? existing.pastor
          : normalizeJsonObject(pastor, "pastor");
...
// Delete church (super or owning regional_admin)
router.delete(
  "/churches/:id",
  authRequired,
  requireRole("super", "regional_admin"),
  ...
  async (req, res) => {
    ...
    if (!imageUrl && !videoUrl) {
      return res.status(400).json({
        error: "Provide at least one media URL: image_url or video_url",
      });
    }
    ...
    if (expiresAt === undefined)
      return res
        .status(400)
        .json({ error: "expires_in_days must be a positive number" });
...
router.put(
  ...
  async (req, res) => {
    ...
    await db.query(
     ...
     WHERE id = $5`,
      [
        text !== undefined ? text : blog.text,
        imageUrl !== undefined ? imageUrl : blog.image_url,
        videoUrl !== undefined ? videoUrl : blog.video_url,
        expiresAt,
        id,
      ],
    );
...
router.post("/posts", authRequired, async (req, res) => {
  ...
  if (!content || !region_id)
    return res.status(400).json({ error: "Missing content or region_id" });
  ...
  if (!validChurches)
    return res.status(400).json({
      error: "One or more church_ids do not belong to the selected region",
    });
  ...
  if (Array.isArray(church_ids) && church_ids.length > 0) {
    ...
    for (const churchId of uniqueChurchIds) {
      await db.query(
        "INSERT INTO post_churches(post_id,church_id) VALUES($1,$2) ON CONFLICT DO NOTHING",
        [id, churchId],
      );
    }
  }
...
// Update post (super or regional admin)
router.put("/posts/:id", authRequired, async (req, res) => {
  ...
  const user = req.user;
  if (user.role === "regional_admin") {
    const allowedRegion = await userCanPostToRegion(user.id, post.region_id);
    if (!allowedRegion)
      return res.status(403).json({ error: "Forbidden for this region" });
  } else if (user.role !== "super") {
    return res.status(403).json({ error: "Forbidden" });
  ...
  const resolvedRegionId = region_id || post.region_id;
  if (category && !ALLOWED_CATEGORIES.includes(category)) {
    return res
      .status(400)
      .json({
  ...
  if (church_ids !== undefined) {
    const validChurches = await validateChurchIdsForRegion(
      church_ids,
      resolvedRegionId,
    );
    if (!validChurches)
      return res
        .status(400)
        .json({ error: "One or more church_ids do not belong to the region" });

    await db.query("DELETE FROM post_churches WHERE post_id=$1", [id]);
    const uniqueChurchIds = [...new Set(church_ids)];
    for (const churchId of uniqueChurchIds) {
      await db.query(
        "INSERT INTO post_churches(post_id,church_id) VALUES($1,$2) ON CONFLICT DO NOTHING",
  ...
  await db.query(
    ...
    [
      ...
      location_link !== undefined ? location_link : post.location_link,
      expiresAt,
      id,
    ],
  );
...
// Delete post (super or regional admin)
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!region_id || !image_url)
      return res.status(400).json({ error: "Missing region_id or image_url" });
    ...
    if (church_id) {
      const validChurch = await db.query(
        "SELECT id FROM churches WHERE id=$1 AND region_id=$2",
        [church_id, region_id],
      );
      if (validChurch.rowCount !== 1)
        return res
          .status(400)
          .json({ error: "church_id is invalid for selected region" });
    }
